...
function isCreditCardNumber(ccn) {
  const stevedoh = String(ccn).split('').reverse();
  const sum = stevedoh.reduce((acc, n, index) => {
    const num = Number(n);
    if (index % 2) {
...
function getDigitalRoot(num) {
  const number = `${num}`.split('').reduce((prev, next) => +prev + +next);
...
function isBracketsBalanced(str) {
  let steVEDOH = str;
  do {
    steVEDOH = steVEDOH.replace('()', '')
      .replace('{}', '')
      .replace('[]', '')
      .replace('||', '')
      .replace('<>', '');
  }
  while (/\(\)/.test(steVEDOH) || /\[\]/.test(steVEDOH) || /\{\}/.test(steVEDOH)
  || /\|\|/.test(steVEDOH) || /<>/.test(steVEDOH));
  if (steVEDOH.length === 0) return true;
  return false;
}
...
function getMatrixProduct(m1, m2) {
  const stevedoh = new Array(m1.length);
